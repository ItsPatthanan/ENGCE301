...
function App() {
  ...
  const movieSearch = (e) => {
    ...
    searchMovie(search_data.search_text).then((movies) => {
  ...
  const movieDelete = (e) => {
    console.log(delete_data.deleteMovie);
    deleteMovie(delete_data.deleteMovie).then((movies) => {
      console.log(movies.data);
      setNumberOfMovies(numberOfMovies - 1);
    });
    fetchAllMovies();
  ...
  const onChangeForm3 = (e) => {
    if (e.target.name === "delete_text") {
      delete_data.deleteMovie = e.target.value;
    }
